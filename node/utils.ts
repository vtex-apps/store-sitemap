...
export const xmlTruncateNodes = (
  xml: string[],
  limit: number = MAX_CALL_STACK_SIZE
...
export const getBucket = (prefix: string, bucketName: string) =>
  `${prefix}_${bucketName}`
...
export const startSitemapGeneration = async (ctx: Context, force?: boolean) => {
  ...
  const config = await vbase.getJSON<GenerationConfig>(
    CONFIG_BUCKET,
