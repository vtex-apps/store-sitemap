...
const reduceByEntity = (array: string[]) =>
...
const groupEntityEntries = async (
  entity: string,
  files: string[],
  ...
  const {
    clients: { vbase, vbaseWithCache },
    vtex: { logger },
  } = ctx
  const lastFile = index && last(index)
  const lastFileData = lastFile
    ? await vbaseWithCache.getJSON<SitemapEntry>(bucket, lastFile)
    : { routes: [] as Route[] }
...
export async function groupEntries(
  ...
  const {
    body,
    clients: { tenant, vbase, vbaseWithCache },
    vtex: { logger },
    state: { enabledIndexFiles },
  } = ctx
  const { indexFile, generationId, from }: GroupEntriesEvent = body

  const { generationPrefix, productionPrefix } =
    (await vbaseWithCache.getJSON<Config>(CONFIG_BUCKET, CONFIG_FILE, true)) ||
    DEFAULT_CONFIG
  const storeBindings = await getStoreBindings(tenant)

  const isBindingGroupingComplete = await Promise.all(
    storeBindings.map(async binding => {
      ...
      const { index: newIndex } =
        from === 0
          ? { index: [] as string[] }
          : (await vbase.getJSON<SitemapIndex>(bucket, indexFile, true)) || {
              index: [] as string[],
            }
      ...
      const index = uniq(
        entries.reduce((acc, entryList) => acc.concat(entryList), newIndex)
      )
