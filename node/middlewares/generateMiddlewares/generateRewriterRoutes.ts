...
const createRoutesByBinding = (
  ...
  return routes.reduce((acc, internal) => {
...
const completeRoute = (rewriter: Rewriter, type: string) => async (
  route: Route
) => {
  ...
  const alternates = routesById.map(({ route: path, binding: bindingId }) => ({
    path,
    bindingId,
  }))
  return {
...
const saveRoutes = (
  routesByBinding: RoutesByBinding,
  count: number,
  clients: Clients
  ...
  const newEntries = await Promise.all(
    Object.keys(groupedRoutes).map(async entityType => {
      const entityRoutes = await Promise.all(
        routesByBinding[bindingId][entityType].map(
          completeRoute(rewriter, entityType)
        )
      )
...
export async function generateRewriterRoutes(
  ...
  const routesByBinding = createRoutesByBinding(
    routes,
    report,
    storeBindings,
    disableRoutesTerm
  ...
  await Promise.all(
    Object.keys(routesByBinding).map(
      saveRoutes(routesByBinding, count, ctx.clients)
    )
  )
