...
export class BindingResolver {
  ...
  public async discoverId(ctx: Context): Promise<string | undefined> {
    const { clients: { tenant: tenantClient }, vtex: { account } } = ctx
    ...
    const matchingBinding = tenantInfo ? this.resolve(ctx, tenantInfo.bindings) : null

    return matchingBinding?.id
  }

  protected findMatchingBinding (account: string, hostAndPath: string, bindings: Binding[]): Binding | null {
    const matching = bindings.reduce((currentWinner, candidate) => {
      ...
      const allCandidateAddresses = [
        ...
        ...PUBLIC_DOMAINS.map(domain => `${account}.${domain}/${canonicalBaseAddress}`),
      ].map(ensureEndingSlash)
      const allMatchingAddress = allCandidateAddresses.filter(address => hostAndPath.startsWith(address))
      if (allMatchingAddress.length === 0) {
        return currentWinner
      }

      const [bestMatchingAddress] = allMatchingAddress.length === 1
        ? allMatchingAddress
        : allMatchingAddress.sort((addressA, addressB) => addressB.length - addressA.length)
  ...
  protected mountHostAndPath(ctx: Context): string {
    ...
    const isInternalPublicDomain = PUBLIC_DOMAINS.some(domain => forwardedHost.endsWith(`${account}.${domain}`))
    const bindingPath = isInternalPublicDomain
      ? this.toBindingPath(forwardedPath)
      : this.toBindingPath(rootPath)
    const resolvedHost = isInternalPublicDomain && forwardedHost.startsWith(workspacePrefix)
  ...
  private resolve (ctx: Context, bindings: Binding[]): Binding | null {
    const {vtex: {account, binding}, query: {__bindingAddress, __bindingId}} = ctx
  ...
  private getBindingByAddress(bindindAddress: string, bindings: Binding[]): Binding {
    const binding = bindings.find(item => item.canonicalBaseAddress === bindindAddress)
    if (!binding) {
