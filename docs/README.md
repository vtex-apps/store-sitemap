...
8. Create a new request and use the `GET` method to send a request to the following URL: `https://{{workspace}}--{{account}}.myvtex.com/_v/generate-sitemap`. The expected response body is an `OK` in text format. This means your sitemap will be available in some minutes, after being processed and saved on our database.

:information_source: If the sitemap generation is already taking place the following message will appear: 
`Sitemap generation already in place
Next generation available: <End-date>`
But you can force the generation by passing the querystring `__force`
