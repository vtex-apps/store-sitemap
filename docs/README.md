...
8. Create a new request and use the `GET` method to send a request to the following URL: `https://{workspace}--{account}.myvtex.com/_v/generate-sitemap`. Once the process starts, the following message will be displayed:
...
Meaning that, after being processed and saved on our database, your store's sitemap will be available soon.
...
After the process is completed, a body response containing an `OK` in text format will be provided.

*:warning: To make a force restart, add the `__force` query string at the end of the API URL, and send a new request.*
